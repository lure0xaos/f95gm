...
internal suspend fun getJson(path: String): String {
    val response = apiRequest(path) { http.get(path) }
    val body = response.bodyAsText()
    if (response.status.value !in f95gm.domain.api.F95HttpStatus.SUCCESS_MIN..f95gm.domain.api.F95HttpStatus.SUCCESS_MAX) {
        error(readApiError(body, UiMessages.app_catalogRequestFailed(response.status.value)))
    }
    return body
}
